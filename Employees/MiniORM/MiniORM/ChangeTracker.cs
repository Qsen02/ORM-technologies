namespace MiniORM
{
    ...
    internal class ChangeTracker<T> where T : class, new() 
    {
        ...
        public IEnumerable<T> GetModifiedEntities(DbSet<T> dbSet) { 
            ...
            PropertyInfo[] primaryKeys = typeof(T).GetProperties().Where(pk => pk.HasAttribute<KeyAttribute>()).ToArray();

            foreach (T proxyEntitty in this.AllEntities)
            {
                object[] primaryKeyValues = GetPrimaryKeyValues(primaryKeys, proxyEntitty).ToArray();

                T entity = dbSet.Entities.Single(e => GetPrimaryKeyValues(primaryKeys, e).SequenceEqual(primaryKeyValues));
                bool isModified = IsModified(proxyEntitty, entity);
                if (isModified) { 
